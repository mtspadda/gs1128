...
export function encodeCode128(text) {
  ...
  for (const ch of text) {
    ...
    if (v === undefined) throw new Error(`Caractere '${ch}' não suportado`);
    values.push(v);
  }
  let sum = START_B;
  ...
  values.push(sum % 103);
  values.push(STOP);
  return values.map((v) => PATTERNS[v]).join("") + "11";
}

export function drawBarcode(canvas, text, label) {
  ...
  canvas.width = encoded.length * barWidth + padX * 2;
